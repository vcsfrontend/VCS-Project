...
export class OptimizerComponent extends BaseComponent {
  ...
  partshistoryDisplayedColumn: string[] = ['slNo', 'sheetId', 'sheetName', 'contentType', 'uploadedBy', 'uploadedTime', 'email', 'recordsCount'];

  dataSource = new MatTableDataSource<any>(); mailId: any = '';
  stockDataSource = new MatTableDataSource<any>();
  sawDataSource = new MatTableDataSource<any>();
  partsHistoryDataSource = new MatTableDataSource<any>();
  ...
  partsHistoryGetSNo(index: number): number {
  ...
  getPartshistory() {
    ...
    this.switchService.HistoryUploadParts(payload).subscribe({
      next: (res: any) => {
        if (res && res.length > 0) {
          this.partsHistoryDataSource.data = res;
          console.log("Stock Data:", this.partsHistoryDataSource.data);
          this.partsHistoryDataSource.paginator = this.partshistoryPaginator;
        } else {
          this.toastr.error("No data received from server");
          this.partsHistoryDataSource.data = [];
        }
      },
      error: (error) => {
        console.error("API Error:", error);
